...
test.describe("Lightspeed UI", () => {
  ...
  test.describe("Lightspeed page", () => {
    ...
    test("page is available", async () => {
      ...
      await expect(
        page.getByRole("heading", { name: "Developer Hub Intelligent Assistant" }),
      ).toBeVisible();
    ...
    test("default prompts are visible", async () => {
      const userName = process.env.RHDH_DISPLAY_NAME?.trim();
    ...
    test("file attachment rejects unsupported file types", async () => {
      const filePath = path.join(e2eRoot, "playwright.config.ts");
      await uploadFiles(page, [filePath]);
      await validateFailedUpload(page);
      await expect(
        page.locator("span", { hasText: "playwright" }).first(),
      ).toBeHidden();
    });
