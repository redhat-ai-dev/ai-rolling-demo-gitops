...
test.describe("Lightspeed notebooks", () => {
  ...
  // Can be unskipped once https://redhat.atlassian.net/browse/RHDHBUGS-3302 is fixed.
  test.skip("upload modal: drop zone, disabled add, and duplicate file prompt", async () => {
  ...
  // Can be unskipped once https://redhat.atlassian.net/browse/RHDHBUGS-3302 is fixed.
  test.skip("sidebar: add file then remove", async () => {
  ...
  // Can be unskipped once https://redhat.atlassian.net/browse/RHDHBUGS-3302 is fixed.
  test.skip("upload modal: eleven files rejected at cap", async () => {
    await notebooks.clickOpenUploadDocumentModal();
    const uploadModal = notebooks.uploadDocumentModal();
  ...
  // Can be unskipped once https://redhat.atlassian.net/browse/RHDHBUGS-3302 is fixed.
  test.skip("upload modal: unsupported extension rejected", async () => {
    await notebooks.clickOpenUploadDocumentModal();
    const uploadModal = notebooks.uploadDocumentModal();
    await uploadModal.selectFilesViaBrowsePicker([
      notebookUnsupportedTypeFixturePath(),
  ...
  test.skip("notebook tab: conversation, feedback, clipboard, and delete notebook", async () => {
    await notebooks.gotoFullscreenNotebooksTab();
    await notebooks.clickCreateNotebookFromEmptyList();
    await expect(page).toHaveURL(NOTEBOOK_EDITOR_URL_RE);
