...
async function getImpersonationCookies(): Promise<BrowserCookie[]> {
  ...
  try {
    ...
    // Legacy Keycloak uses /auth/realms/...; newer installs use /realms/...
    const realmsIdx = realmPath.indexOf("/realms/");
    if (realmsIdx < 0) {
      throw new Error(
        `Unable to derive Keycloak context path from auth URL path: ${parsedAuthUrl.pathname}`,
      );
    }
    const contextPath = realmPath.slice(0, realmsIdx);
    const adminRealmBase = `${origin}${contextPath}/admin/realms/${realmName}`;
    ...
    const usersResp = await api.get(`${adminRealmBase}/users`, {
    ...
    const impersonateResp = await api.post(
      `${adminRealmBase}/users/${users[0].id}/impersonation`,
      {
        headers: { Authorization: `Bearer ${token}` },
        maxRedirects: 0,
      },
